...
export default function HomePage() {
  ...
  const [trendingWords, setTrendingWords] = useState<{word: string}[]>([]);

  const searchContainerRef = useRef(null);

  // Fetch random words on component mount (once per session)
  useEffect(() => {
    fetch("/api/random")
      .then(res => res.json())
      .then(data => {
  ...
  return (
      ...
      {/* Central Search Area */}
        ...
        {/* Keyboard Mode Selectors */}
          ...
          {[
            ...
            // ইউনিবিজয় removed as requested
          ].map((mode) => (
             setKeyboardMode(mode.id as KeyboardMode)}
      ...
      {/* Featured Sections - Equal Size Grid */}
            ...
            {wordOfDay ? (
              <>
                ...
                {wordOfDay.pronunciation && (
                  উচ্চারণ: {wordOfDay.pronunciation}
                )}
